...
class FNHostelMessController extends AdminController {
	 ...
	 */
     ...
     public function hm_voucher_approval_persons_edit(){
        ...
        if($this->input->post('btn-update')):
            ...
            redirect('HmApprovalPersons');
                                                    ...
                                                    $this->db->join('fn_account_types','fn_account_types.id=fn_vocher_approvalby.fn_account_type_id');    
                                                    $this->db->join('hr_emp_record','hr_emp_record.emp_id=fn_vocher_approvalby.fn_va_emp_id','left outer');    
        $this->data['result_query']           =     $this->db->get_where('fn_vocher_approvalby',array('fn_account_type_id'=>2,'fn_vocher_approvalby.id'=>$this->uri->segment(2)))->row();
        $this->data['financialYear']          = $this->CRUDModel->dropDown('financial_year','', 'id', 'year',array('id'=>$this->data['result_query']->fn_va_fy_id,'fn_account_type_id'=>2)); 
//        echo '<pre>';print_r($this->data['result_query']);die;
        
        $this->data['page']             = "Finance/HM/Setups/hm_finance_voucher_approval_edit";
        $this->data['page_heading']     = "HM Approval Persons Edit";
        $this->data['page_title']       = 'HM Approval Persons Edit | ECMS';
        $this->load->view('common/common',$this->data);
    }
...
//            $this->data['stt']              = $this->CRUDModel->dropDown('fn_trm_type','', 'ftt_id', 'ftt_title');  
   ...
   public function hm_save_vouchers(){
         ...
         if($this->input->post()):
           ...
           if(!empty($voucher_att)):
            ...
            endforeach;
              ...
              $get_apvl_persons = $this->CRUDModel->get_where_result('fn_vocher_approvalby',array('status'=>1,'fn_account_type_id'=>2));
    ...
    public function hm_print_vouchers(){
        ...
        $order['order']   = 'asc';
        $this->data['approval']         = $this->CRUDModel->get_where_result_order('gl_at_aprove_by',array('trns_ab_status'=>1,'trns_ab_amount_trans_id'=>$voucher_id,'trns_ab_account_type_id'=> 2),$order);
...
public function hm_vouchers_sign_by(){
                ...
                foreach($chk_trns as $row=>$key):
                    $sing_persons = $this->CRUDModel->get_where_row('fn_vocher_approvalby',array('id'=>$key));
                     $aprl_data = array(
                          ...
                          $this->CRUDModel->insert('gl_at_aprove_by',$aprl_data);
            endforeach;
                redirect('HmVoucherSign/'.$this->input->post('transactions_id'));
            endif;
        ...
        $voucher_id = $this->uri->segment(2);
                                                ...
                                                $this->db->join('hr_emp_record','hr_emp_record.emp_id=fn_vocher_approvalby.fn_va_emp_id','left outer'); 
                                                $this->db->order_by('appr_order','asc');
        $this->data['upd_apr_by']           =   $this->db->get_where('fn_vocher_approvalby',array('status'=>1,'fn_account_type_id'=>2))->result();
        ...
        $this->data['page']             = "Finance/HM/Forms/hm_finance_sign_by_v";
        $this->data['page_heading']     = "HM Voucher Sign By";
        $this->data['page_title']       = 'HM Voucher Sign By | ECMS';
        $this->load->view('common/common',$this->data);
    }
    
    public function hm_vouchers_sign_by_delete(){
        $deletId = $this->uri->segment(2);
        
       $this->CRUDModel->deleteid('gl_at_aprove_by',array('trns_ab_id'=>$deletId));
       redirect('HmVoucherSign/'.$this->uri->segment(3));
    }
}
