                                                           ...
                                                           $sn++;
                                                            ...
                                                            if($row->status == 1):
                                                              ...
                                                              if(empty($checkValue)):
                                                                  ...
                                                                  echo '<td><button type="button" class="btn btn-theme btn-xs disabled"> Delete </button></td>';
                                                              endif;
...
//            echo '<pre>';print_r($this->data['result']);die;
    ...
    public function hostel_mess_payment(){
        ...
        if($this->input->post('save')):
           ...
           $challan_info            = $this->HostelModel->challan_info($where_ch_info);
            ...
            foreach($challan_info as $chRow):
                    $paid       = '0';   
                    $balance    = '0';
                if($check_lock->challan_lock == 1):
                    $paid       = $chRow->amount;   
                    $balance    = '0';   
                else:
                    $balance    = $chRow->amount-$chRow->balance;   
                    $paid       = $chRow->balance;   
                endif;
            $data_info = array(
                    'paid_amount'   => $paid,
        ...
        $this->data['page']         = 'hostel/Forms/hostel_mess_payment_v';
        $this->load->view('common/common',$this->data);
    }
    public function hostel_mess_installment(){
        ...
        if($this->input->post('search')):
            ...
            $this->data['challan_id']   = $challan_id;
                                          ...
                                          $this->db->limit('1','0');
            $check_last_challan         = $this->db->get_where('hostel_student_bill',array('head_type'=> $this->data['std_info']->head_type,'hostel_student_bill.hostel_std_id' => $this->data['std_info']->hostel_id))->row();
            if($this->data['std_info']->challan_status == 2):
                $this->data['error_msg'][] = 'Sorry! Challan already Paid on Date'.date('d-M-Y',strtotime($this->data['std_info']->payment_date));
            endif;
            if($check_last_challan->id == $challan_id):
                    $this->data['challan_info'] = $this->HostelModel->challan_info($where);
                else:
                    $this->data['error_msg'][] = 'For challan installment select last challan';
            endif;
          endif;
        
        if($this->input->post('save')):
                $challan_id         = $this->input->post('challan_no'); 
                $hostel_std_id      = $this->input->post('hostel_std_id'); 
                $installmentAmount  = $this->input->post('InstallmentAmount');
                $challDetId         = $this->input->post('ChlnDetailId');
                //Update Challan table to installment flag to 1= installment, 0 = normal challan
                $this->CRUDModel->update('hostel_student_bill',array('installment_flag'=>1),array('id'=>$challan_id));
                $combine            = array_combine($challDetId,$installmentAmount);
                    
//                     echo '<pre>';print_r($challId);die;
                foreach($combine as $row=>$key):
                    $data   = array('balance' =>$key  );
                    $set    = array('id'=>$row);
                    $this->CRUDModel->update('hostel_student_bill_info',$data,$set);
                endforeach;
                redirect('hostelPrintChallan/'.$hostel_std_id.'/'.$challan_id,'refresh');
        endif;
        
        $this->data['page_title']   = 'Hostel & Mess Installment | ECMS';
        $this->data['page_header']  = 'Hostel & Mess Installment';
        $this->data['page']         = 'hostel/Forms/hostel_mess_installment';
        $this->load->view('common/common',$this->data);
    }
...
//            $where['hostel_student_bill.challan_status'] = 2;
            ...
            if(!empty($bank)):
                $where['hostel_student_bill.bank_id']   = $bank;
                $this->data['bank_id']                  = $bank;
                $this->data['bank_name']                = $this->db->get_where('bank',array('bank_id'=>$bank))->row();
...
//           echo '<pre>';print_r($fee_head_info);/
        ...
        if($id):
            ...
            $this->data['hostel_head_title']        = $this->CRUDModel->dropDown('hostel_head_title','', 'id', 'title',array('id'=>$row->head_title_id,'status'=>'1'));
      ...
      public function hostel_fee_group(){
            $this->data['bank_hostel']          = $this->DropdownModel->bank_dropDown('bank','', 'bank_id', 'name',array('category_type'=>1)); 
            $this->data['default_bank']         = $this->DefaultFeeBank->bank_id;
...
//                redirect('NewAdmissonInterSearch');
        ...
        public function hostel_new_record_admin_fee(){
        $student_id                     =   $this->uri->segment(2);
        $this->data['result']           = $this->HostelModel->hostel_new_record(array('student_record.student_id'=>$student_id));
        $this->data['hostel_batch']     =   $this->CRUDModel->dropDown('hostel_batch', ' Hostel Session ', 'h_batch_id', 'h_batch_name');  
        $this->data['relation']         =   $this->CRUDModel->dropDown('relation', ' Relection', 'relation_id', 'title');  
//        echo '<pre>';print_r($this->data['result']);die;
        ...
        if($this->input->post()):
            ...
            $cnic               = $this->input->post('cnic');
                ...
                $this->CRUDModel->insert('hostel_student_record',$data);
//                redirect('NewAdmissonInterSearch');
            redirect('HostelController/enrolled_student_record');
...
//        $this->data['result'] = $this->HostelModel->get_HostelData('hostel_student_record');
        ...
        if($this->input->post('export')):
                ...
                $this->excel->getActiveSheet()->setCellValue('I1', 'Allotted Date');
                $this->excel->getActiveSheet()->getStyle('I1')->getFont()->setBold(true);
                $this->excel->getActiveSheet()->getStyle('I1')->getFont()->setSize(16);
        
                $this->excel->getActiveSheet()->setCellValue('J1', 'Block');
                $this->excel->getActiveSheet()->getStyle('J1')->getFont()->setBold(true);
                $this->excel->getActiveSheet()->getStyle('J1')->getFont()->setSize(16);
        
                $this->excel->getActiveSheet()->setCellValue('K1', 'Room');
                $this->excel->getActiveSheet()->getStyle('K1')->getFont()->setBold(true);
                $this->excel->getActiveSheet()->getStyle('K1')->getFont()->setSize(16);
                ...
                $this->excel->getActiveSheet()->setCellValue('M1', 'T.Marks');
                $this->excel->getActiveSheet()->getStyle('M1')->getFont()->setBold(true);
                $this->excel->getActiveSheet()->getStyle('M1')->getFont()->setSize(16);
        
                $this->excel->getActiveSheet()->setCellValue('N1', 'Shift');
                $this->excel->getActiveSheet()->getStyle('N1')->getFont()->setBold(true);
                $this->excel->getActiveSheet()->getStyle('N1')->getFont()->setSize(16);
        
                $this->excel->getActiveSheet()->setCellValue('O1', 'Admission In');
                $this->excel->getActiveSheet()->getStyle('O1')->getFont()->setBold(true);
                $this->excel->getActiveSheet()->getStyle('O1')->getFont()->setSize(16);
...
//            $this->data['s_status']     = $this->CRUDModel->dropDown('student_status', 'Select Status', 's_status_id', 'name');
           ...
           $this->data['page']         = 'hostel/all_student_record_hostel';
           $this->load->view('common/common',$this->data);
    }
...
//                endif;
               if(!empty($section)):
                ...
                $limitVale = $this->CRUDModel->get_where_row('show_limit',array('limitId'=>$limit));
                    ...
                    $this->data['page']     = "hostel/search_enrolled_student_record";
   ...
   public function change_date_paid_challan(){
        ...
        $this->data['page']         = 'hostel/Forms/date/hm_challan_update_date';
        $this->data['page_header']  = 'Challan date ( Paid Challan )';
        $this->data['page_title']   = 'Challan date ( Paid Challan )| ECMS';
        $this->load->view('common/common',$this->data);
    }
        public function change_challan_date_grid(){
        ...
        if($this->input->post('request') == 'SearchStudents'):
            ...
            $amount             = $this->input->post("amount");
                ...
                $this->data['result']                       = $this->HostelModel->hostel_mess_paid_challan_change_date($where,$amount,$like);
             ...
             $return_json = array();   
                ...
                if($challanIds):
                    foreach($challanIds as $challanId):
                         $this->CRUDModel->update('hostel_student_bill',array('valid_date'=>date('Y-m-d',strtotime($this->input->post('due_date')))),array('id'=>$challanId));
