...
class ApiController extends AdminController{
	function __construct(){
        ...
        public function live_api_student_info(){
            ...
            $this->data['studentStatus']= $this->CRUDModel->dropDown('student_status', 'Student Status', 's_status_id', 'name');
            ...
            if($this->input->post('CreateAuth')):
                  ...
                  $this->data['studentStatus_id']   = $this->input->post('studentStatus');
...
//                    'applicant_image'       => $row->applicant_image,
                ...
                // Student Grades
                if(!empty($row->Student_Grades)):
                    foreach($row->Student_Grades as $GrdRow):
                        $GradesInfo = array(
                          'student_id'         => $student_id, 
                          'grade_id'           => $GrdRow->grade_id, 
                          'ol_sub_pass_year'   => $GrdRow->ol_sub_pass_year, 
                          'ol_subject_id'      => $GrdRow->ol_subject_id, 
                          'programe_id'         => $GrdRow->programe_id, 
                          'sub_pro_id'          => $GrdRow->sub_pro_id, 
                        );
                         $this->CRUDModel->insert('student_grades',$GradesInfo); 
                    endforeach;
                endif;
                ...
                if(!empty($row->Student_Fee_Info)):
                    foreach($row->Student_Fee_Info as $FeeRow):
                        $Fee_Details = array(
                          ...
                          'pros_amount'         => $FeeRow->pros_amount,  
                          'pros_paid_by'        => $FeeRow->pros_paid_by,  
                          'pros_bank_id'        => $FeeRow->pros_bank_id,  
                          'pros_paid_status'    => $FeeRow->pros_paid_status,   
                        );
