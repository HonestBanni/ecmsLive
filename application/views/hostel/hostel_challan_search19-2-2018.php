                            ...
                            $grandTotal = '';
                             $actual_amountTotal = '';
                              ...
                              echo '<tr">
                                      ...
                                      <td><a href="fullDetails/'.$row->student_id.'" target="_new"><strong title="'.$row->student_name.'/'.$row->father_name.', Class :('.$row->sessionName.'), Student status = '.$row->student_status.'") target="_new">'.substr($row->student_name, 0, 15).'</strong></a></td>    
...
//                                            <td>'.$row->sub_program_name.'&nbsp;&nbsp;'.$row->sessionName.'&nbsp;&nbsp;'.$row->batch_name.'</td>
                                      ...
                                      echo '<td>'.$row->status_name.'</td><td>'.$row->fcs_title.'</td><td>'.$row->Category_title.'</td><td>'.date('d-M-y',  strtotime($row->date_from)).'/'.date('d-M-y',  strtotime($row->date_to)).'</td>';

//                                    echo '<pre>';print_r($where);die;
                                      $where['hostel_student_bill.id'] = $row->challan_id;
                                      $cons =   $this->HostelModel->get_hotel_amount($where); 
                                      $balance = $cons->actual_amount-$cons->total_paid;
                                      
                                      echo  '<td>'.$row->challan_id.'</td>
                                                
                                                <td>'.$cons->total_paid.'</td>
                                    ...
                                    if($row->payment_date == '0000-00-00'):
                                       ...
                                       echo '<td>'.$payment_date.'</td>
                                                ...
                                                if($row->ch_status_id ==1):
                                                    echo '
                                                    <td style="font-size:11px;"><a href="hostelPrintChallan/'.$row->hostel_id.'/'.$row->challan_id.'" class="btn btn-success btn-xs" target="_new"><i class="fa fa-print"></i>Print<a></td>
                                                    ...
                                                    <td style="font-size:11px;"><a href="hostelUpdateChallan/'.$row->hostel_id.'/'.$row->challan_id.'" class="btn btn-warning btn-xs" target="_new"><i class="fa fa-book"></i>Update<a></td>
