...
//                        echo '<pre>';print_r($balace_info);
                         ...
                         if(empty($balace_info)):
                             echo '<td colspan="2" style="border: 1px solid #000000;"></td>';
                                 ...
                                 if($ch_Row->installment_flag == 1):
                                            if($extra_info->challan_status == 2):
                                            echo '<td colspan="2" style="border: 1px solid #000000;">'.$balace_info->paid_amount.'</td>';
                                            $balance =  $balace_info->paid_amount;    
                                                else:
                                            echo '<td colspan="2" style="border: 1px solid #000000;">'.$balace_info->balance.'</td>';
                                            $balance =  $balace_info->balance;    
                                            endif;
...
//                         echo '<pre>';print_r($current_info);
                         ...
                         if(empty($current_info)):
                             echo '<td colspan="2" style="border: 1px solid #000000;"></td>';
                                  ...
                                  if($ch_Row->installment_flag == 1):
                                      if($extra_info->challan_status == 2):
                                            echo '<td colspan="2" style="border: 1px solid #000000;">'.$current_info->paid_amount.'</td>';
                                            $current =  $current_info->paid_amount;  
                                          else:
                                            echo '<td colspan="2" style="border: 1px solid #000000;">'.$current_info->balance.'</td>';
                                            $current =  $current_info->balance;  
                                      endif;
...
//                         echo '<td colspan="2" style="border: 1px solid #000000; text-align: right;">'.$ch_Row->amount.'</td> ';
                ...
                <table class="table table-hover" id="table" style="margin-bottom: 1px; font-size: 12px;">    
                    <tbody>
                        <?php
                            if($extra_info->installment_flag == 1):
                                echo '<tr><td style="border: 1px solid #000000;background: #000;color: #fff;text-align: center;">Installment Challan</td></tr>';
                            endif;
               ...
               <ol class="custom-list-style">
                    ...
                    <li><strong>No Fee shall be deposited after <?php echo date('d-M-Y',strtotime($extra_info->valid_date))?>.</strong></li>

                </ol>
<!--               <strong>Instruction For Bank</strong>
               <p>Fee can be paid at any HBL-Branch<br/>
               Please mention challan# as "Mandatory" wile depositing the fee, so that can easily be traced in band statement.<br/>
               No Fee shall be deposited after <strong><?php echo date('d-M-Y',strtotime($extra_info->valid_date))?></strong></p>-->
                </div>
              </div>
            </div>
          </div>
