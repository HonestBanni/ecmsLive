...
<h2 align="left">Update Employee Academic ( <?php echo $grec->emp_name; ?> )<hr></h2>
    ...
    <h4 style="color:green; text-align:center;"><?php print_r($this->session->flashdata('insert_msg'));?></h4> 
                        ...
                        <br />
<form method="post" enctype="multipart/form-data" action="HrController/update_emp_edu/<?php echo $emp_edu_id;?>">    
...
$gres = $this->HrModel->get_by_id('board_university',array('bu_id'=>$bu_id));
    if($gres){
        foreach($gres as $grec){ ?>   
    <input type="text" name="bu_id" value="<?php echo $grec->title;?>" class="form-control" placeholder="Type University" id="bu">
    <input type="hidden" name="bu_id" value="<?php echo $grec->bu_id;?>" id="bu_id">
     <?php 
        }     
    }else{
?>
