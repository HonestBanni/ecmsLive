<div class="content container">
    <h2 align="left">Teacher Attendance List<hr></h2>
        <div class="row cols-wrapper">
            <div class="col-md-12">
                <form method="post" action="AttendanceController/search_attendance_history_bs">
            ...
            <div class="form-group col-md-2">
                <?php
                    ...
                    if($gres){
                        ...
                        { ?>          
                        <input type="text" name="sec_id" value="<?php echo $grec->name; ?>" placeholder="Section" class="form-control" id="sec_bs">
                        <input type="hidden" name="sec_id" id="sec_id" value="<?php echo $grec->sec_id; ?>">      
                        <?php 
                        }     
                    }else{?>
                    <input type="text" name="sec_id" placeholder="Section" class="form-control" id="sec_bs">
            ...
            <div class="form-group col-md-2">
                <?php
                    ...
                    if($gres){
                        foreach($gres as $grec)
                        { ?>          
                        <input type="text" name="subject_id" value="<?php echo $grec->title; ?>" placeholder="Subject" class="form-control" id="sub_bs"> 
                        <input type="hidden" name="subject_id" id="subject_id" value="<?php echo $grec->subject_id; ?>">   
                        <?php 
                        }     
                    }else{?>
                        <input type="text" name="subject_id" placeholder="Subject" class="form-control" id="sub_bs">
...
<script>
    jQuery(document).ready(function(){
        ...
        jQuery("#sec_bs").autocomplete({  
            minLength: 0,
            source: "AttendanceController/auto_section_bs/"+$("#sec_bs").val(),
            autoFocus: true,
            scroll: true,
            dataType: 'jsonp',
            select: function(event, ui){
            jQuery("#sec_bs").val(ui.item.contactPerson);
            jQuery("#sec_id").val(ui.item.sec_id);
            }
            }).focus(function() {  jQuery(this).autocomplete("search", "");  });

        jQuery("#sub_bs").autocomplete({  
            minLength: 0,
            source: "AttendanceController/auto_sub_bs/"+$("#sub_bs").val(),
            autoFocus: true,
            scroll: true,
            dataType: 'jsonp',
            select: function(event, ui){
            jQuery("#sub_bs").val(ui.item.contactPerson);
