...
class FinanceModel extends CI_Model
{
    ...
    public function coa_master($table,$where){
          ...
          $query =      $this->db->select('*')
                        ...
                        ->join('fn_account_types','fn_account_types.id=fn_coa_master_child.fn_account_type_id')
    ...
    public function coa_master_subChild($table,$where){
          ...
          $query =      $this->db->select('*')
                        ...
                        ->join('fn_coa_parent','fn_coa_parent.fn_coaId=fn_coa_master_child.fn_coa_m_pId')
                        ->join('fn_account_types','fn_account_types.id=fn_coa_master_sub_child.fn_account_type_id')
                        ->where($where)
...
//                       $this->db->where('gl_ad_coa_mc_pk BETWEEN "'.$fromCode.'" and "'.$toCode.'"');
                   ...
                   $this->db->where('gl_amount_transition.fn_account_type_id', 1);
//                    $this->db->distinct('gl_ad_coa_mc_pk');
                    $this->db->group_by('gl_ad_coa_mc_pk');
                    $this->db->order_by('gl_ad_coa_mc_id');
...
public function hm_get_leader_dw($table,$dateFrom=NULL,$dateTo=NULL,$fromCode=NULL,$toCode=NULL){
                   ...
                   $this->db->where('gl_amount_transition.fn_account_type_id',2);
                    $this->db->group_by('gl_ad_coa_mc_pk');
                    $this->db->order_by('gl_ad_coa_mc_id');
         $result =  $this->db->get($table)->result();
...
//       return $return_array;
      ...
      return $this->db->where($where)->get('gl_amount_transition')->result();
        ...
        public function get_amountDetails_balance_sheet($table,$where=NULL,$dateFrom,$dateTo){
            
                   $this->db->join('gl_amount_details','gl_amount_transition.gl_at_id=gl_amount_details.gl_ad_atId');
//                    $this->db->where('payment_date BETWEEN "'.date('Y-m-d', strtotime($dateFrom)).'" and "'.date('Y-m-d', strtotime($dateTo)).'"');
                   $this->db->where('payment_date BETWEEN "'.date('Y-m-d', strtotime('01-07-2016')).'" and "'.date('Y-m-d', strtotime($dateTo)).'"');
//                   $this->db->where('payment_date <',date('Y-m-d', strtotime($dateFrom)));
      ...
      public function get_amountDetails_old($table,$where=NULL,$dateFrom,$dateTo){
                   ...
                   $this->db->join('gl_amount_details','gl_amount_transition.gl_at_id=gl_amount_details.gl_ad_atId');
                    ...
                    $this->db->where('vocher_status',2);
                    $this->db->order_by('gl_ad_date','asc');
                    $this->db->order_by('gl_at_vocher','asc');
      return $this->db->where($where)->get('gl_amount_transition')->result();
...
//                   $this->db->where('gl_ad_date BETWEEN "'.date('Y-m-d', strtotime($query->year_start)).'" and "'.date('Y-m-d', strtotime($query->year_start)).'"');
...
//                    endif;
    ...
    public function full_defaulter_notice_report_search($where=NULL,$like=NULL){
               $this->db->select(
                    '
                        ...
                        sections.sec_id,
                        student_denotice.amount as noticeAmount,
                        student_denotice.print_date,
                        student_denotice.print_date,
                        student_denotice.due_date,
                        student_denotice.dn_id,
                        '
                    );
        ...
        return $this->db->get_where('student_record')->result();
    }
    public function full_defaulter_notice_report_search1($where=NULL){
               $this->db->select(' 
                       sub_programes.name as sub_program,
                       student_record.*,
 ...
 public function student_dnoties_search($table,$where){
        $this->db->select('
                student_record.student_id,
...
//     
  ...
  public function voucher_info($where){
                ...
                $this->db->join('fn_account_types','fn_account_types.id=gl_amount_transition.fn_account_type_id');
  ...
  public function search_date_range($where=NULL,$date=NULL,$like=NULL,$payDate=NULL,$deposit_amount){
...
public function trail_balance_new($table,$dateFrom=NULL,$dateTo=NULL,$fromCode=NULL,$toCode=NULL){
                 ...
                 $this->db->order_by('fn_coa_mc_code','asc');
         return  $this->db->get_where('fn_coa_master_sub_child',array('fn_account_type_id'=>1))->result();
          
   }        
public function trail_balance_new_update_on_14_03_2019($table,$dateFrom=NULL,$dateTo=NULL,$fromCode=NULL,$toCode=NULL){
...
//                    $this->db->where( 'gl_amount_transition.fn_account_type_id', 1);
                    $this->db->where(array('gl_amount_transition.fn_account_type_id'=>1,'vocher_status'=> 2));
         $result =  $this->db->get($table)->result();
...
public function income_statement($dateFrom=NULL,$dateTo=NULL,$fromCode=NULL,$toCode=NULL,$where_income_statu){
...
public function search_result_brs($dateFrom=NULL,$dateTo=NULL,$fromCode=NULL){
...
public function search_result_brs_hm($dateFrom=NULL,$dateTo=NULL,$fromCode=NULL){
   ...
   public function get_BRS_report($where){
        ...
        $result1         = $this->db->get('fn_brs_report')->result();
        $return_array   = '';
        ...
        $details        = $this->db->get_where('fn_brs_report_details',array('brs_id'=>$result->id))->result();
    ...
    $return_array[] = array(
               'for_month'          => $result->for_month,
               'fn_coa_mc_title'    => $result->fn_coa_mc_title,
               'fn_coa_mc_code'     => $result->fn_coa_mc_code,
               'total_amount'       => $total,
               'brs_report_id'      => $result->id,
   ...
   ///******************************************************************************************
   //
   //        HOSTEL & MESS FUNCTIONS
   //        
   ///******************************************************************************************
   
   public function hm_income_statement($dateFrom=NULL,$dateTo=NULL,$fromCode=NULL,$toCode=NULL,$where_income_statu){
...
//              echo '<pre>';print_r($return_array);die;       
   ...
   public function hm_trail_balance_new($table,$dateFrom=NULL,$dateTo=NULL,$fromCode=NULL,$toCode=NULL){
   ...
   public function bank_check_record($where){
                           $this->db->join('gl_bank_cheque_details','gl_bank_cheque_details.gl_bc_id=gl_bank_cheques.gl_bc_id');  
                            $this->db->join('fn_coa_master_sub_child','fn_coa_master_sub_child.fn_coa_mc_id=gl_bank_cheque_details.bank_id');  
//                            $this->db->join('fee_coa_master_sub_child','fee_coa_master_sub_child.fn_coa_mc_id=gl_bank_cheque_details.bank_id');  
                            $this->db->join('gl_bank_cheque_type','gl_bank_cheque_type.gl_ct_id=gl_bank_cheque_details.check_type');  
                            $this->db->join('gl_bank_cheques_status','gl_bank_cheques_status.gl_bcs_id=gl_bank_cheque_details.checque_status');  
                            $this->db->order_by('gl_bank_cheques.gl_bc_id','asc');
        return  $this->db->get_where('gl_bank_cheques',$where)->result();
   }
}
