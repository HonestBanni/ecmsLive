...
//                $this->db->order_by('fc_challan_id','asc');
    ...
    public function student_fine_info($where){
                $this->db->select(
                        ...
                        fee_fine.fine_date,
                        fee_fine.fine_comments,
                        fee_fine.fine_status,
                        fee_fine.amount,
                        fee_fine.fh_id,
                        fee_heads.fh_head,
                        fee_fine_status.fine_title,
                        fee_fine.id as fee_id
                        '
                        );
                 $this->db->join('fee_fine_status','fee_fine_status.id=fee_fine.fine_status');
                 $this->db->join('fee_heads','fee_heads.fh_Id=fee_fine.fh_Id');
                 $this->db->order_by('fee_fine.id','desc');
         return  $this->db->where($where)->get('fee_fine')->result();
    }  
