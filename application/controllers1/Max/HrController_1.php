...
public function add_acr(){	  	
  ...
  public function register_employee(){
            ...
            $this->data['gender']       = $this->CRUDModel->dropDown('gender', 'Gender', 'gender_id', 'title');
            $this->data['status']       = $this->CRUDModel->dropDown('hr_emp_status', 'Status', 'emp_status_id', 'title');
            $this->data['network']      = $this->CRUDModel->dropDown('mobile_network', 'Select', 'net_id', 'network');
            ...
            $this->data['bank']         = $this->CRUDModel->dropDown('bank', 'Bank', 'bank_id', 'name');
...
//            $date3 = date('Y-m-d', strtotime($this->input->post('retirement_date')));
            $data = array(
                'emp_name'              => ucwords(strtolower(ucwords($this->input->post('emp_name')))),
                'father_name'           => ucwords(strtolower(ucwords($this->input->post('father_name')))),
                'emp_husband_name'      => ucwords(strtolower(ucwords($this->input->post('emp_husband_name')))),
                'gender_id'             => $this->input->post('gender_id'),
                ...
                'retirement_date'       => date('Y-m-d', strtotime($this->input->post('retirement_date'))),
                'cat_id'                => $this->input->post('cat_id'),
                'c_emp_scale_id'        => $this->input->post('c_emp_scale_id'),
                'current_designation'   => $this->input->post('current_designation'),
                'department_id'         => $this->input->post('department_id'),
            ...
            $UserId = $this->CRUDModel->insert('hr_emp_record',$data);
            redirect('HrController/employee_academic_record/'.$UserId);
        endif;
        
            $this->data['breadcrumbs']  = 'Register Employee';
            $this->data['page_title']   = 'Register Employee | ECP';
            $this->data['page']         = 'hr/Forms/register_employee_v';
            $this->load->view('common/common',$this->data);
...
//                    echo '<pre>';print_r($this->data);die;
            ...
            $config['base_url']         = base_url('EmployeeRecords');
            $config['total_rows']       = count($this->HrModel->get_employee());  //echo $config['total_rows']; exit;
            $config['per_page']         = 50;
            $config["num_links"]        = 6;
