...
<?php
    ...
    public function stock_balance_report()
    {
            ...
            $this->data['item_id']          = "";    
            $this->data['catm_id']          = "";
            $this->data['issuance_from']    = "";   
            $this->data['issuance_to']      = date('d-m-Y');   
            $this->data['emp_id']           = ''; 
            
            
        if($this->input->post('search')):
            $issuance_to        = $this->input->post('issuance_to');
            $issuance_from      = $this->input->post('issuance_from');
            $item_id            = $this->input->post('item_id');
            ...
            if($catm_id):
                ...
                $date['issuance_from']      = $issuance_from;
                $date['issuance_to']        = $issuance_to;
                
            endif;
                if(empty($issuance_from)):
                        $this->data['issuance_from'] = '';
                        else:
                        $this->data['issuance_from'] = $issuance_from;
                    endif;
                    $this->data['issuance_to']  = $issuance_to;
            $this->data['result']   = $this->InventoryModel->get_stocks_balance($where,$date);
//             echo '<pre>';print_r($this->data['result']);die;
        ...
        $result = $this->InventoryModel->get_stocks_balance_excel($where,$date);
                $exceldata="";
        ...
        $this->data['page_header']   = 'Stock Balance Report';
...
//            $this->load->view('common/common',$this->data);  
    ...
    public function autocomplete_items(){
            ...
            if( $term == ''){
                ...
                $labels         = array();
                    foreach ($result_set as $row_set) {
                        $labels[]       = array( 
                            'label'     =>$row_set->itm_name.' (Qty:'.$row_set->item_quantity.'), ('.$row_set->itm_shortname.')', 
                            'code'     =>$row_set->itm_id, 
    ...
    public function autocomplete_items_cons(){
        
        $term = trim(strip_tags($this->input->get('term')));
        if( $term == ''){

            $result_set     = $this->InventoryModel->get_autoComplete_record('invt_items',array('itm_status'=>1, 'itm_asstId'=>2));
            $labels         = array();
                foreach ($result_set as $row_set) {
                    $labels[]       = array( 
                        'label'     =>$row_set->itm_name.' (Qty:'.$row_set->item_quantity.'), ('.$row_set->itm_shortname.')', 
        ...
        $matches                    = array_slice($matches, 0, 10);
            ...
            $result_set             = $this->InventoryModel->get_autoComplete_record('invt_items',array('itm_status'=>1, 'itm_asstId'=>2),array('itm_name'=>$like));
            $labels                 = array();
                foreach ($result_set as $row_set) {
                $labels[]           = array( 
                        'label'     =>$row_set->itm_name.' (Qty:'.$row_set->item_quantity.'), ('.$row_set->itm_shortname.')', 
    ...
    public function autocomplete_deptt(){
        
            $term = trim(strip_tags($this->input->get('term')));
            if( $term == ''){
              
                $result_set     = $this->InventoryModel->get_autoComplete_deptt('department');
                $labels         = array();
                    foreach ($result_set as $row_set) {
                        $labels[]       = array( 
                            'label'     =>$row_set->title, 
                            'code'     =>$row_set->department_id, 
                            'value'     =>$row_set->title, 
                    );
                }
            $matches    = array();
            ...
            $matches                    = array_slice($matches, 0, 10);
                ...
                $result_set             = $this->InventoryModel->get_autoComplete_deptt('department', array('title'=>$like));
                $labels                 = array();
                    foreach ($result_set as $row_set) {
                    $labels[]           = array( 
                            'label'     =>$row_set->title, 
    ...
    public function autocomplete_deptt_inventory(){
        
            $term = trim(strip_tags($this->input->get('term')));
            if( $term == ''){
              
                $result_set     = $this->InventoryModel->autoComplete_deptt_inventory('invt_item_issuance_department');
                $labels         = array();
                    foreach ($result_set as $row_set) {
                        $labels[]       = array( 
                            'label'     =>$row_set->dept_name, 
                            'code'     =>$row_set->iss_dept_id, 
                            'value'     =>$row_set->dept_name, 
                    );
                }
            $matches    = array();
                foreach($labels as $label){
            ...
            $matches                    = array_slice($matches, 0, 10);
                ...
                $result_set             = $this->InventoryModel->autoComplete_deptt_inventory('invt_item_issuance_department', array('dept_name'=>$like));
                $labels                 = array();
                    foreach ($result_set as $row_set) {
                    $labels[]           = array( 
                            'label'     =>$row_set->dept_name, 
...
//        $maxNumberDemo  = $this->CRUDModel->get_max_value('cid_code','invt_consumable_item_details_demo',$where);
        ...
        $itemnameCode   = $this->input->post('itemnameCode');
        $deptt_name     = $this->input->post('depttname');
        $serial_no      = $this->input->post('serial_no');
        $manual_grn     = $this->input->post('manual_grn');
        $quantity       = $this->input->post('quantity');
        $purchaseDate   = date('Y-m-d', strtotime($this->input->post('purchaseDate')));
        $issuedate      = date('Y-m-d', strtotime($this->input->post('issuedate')));
        $formCode       = $this->input->post('formCode');
...
//        $maxNumberDemo  = $this->CRUDModel->get_max_value('cid_code','invt_consumable_item_details_demo',$where);
        ...
        if($quantity>1):
            ...
            for($i=0; $i<$quantity; $i++):
              $data = array(   
                    ...
                    'fid_purchasePrice' => $purchasePrice,
                    'fid_grn'           => $item_grn,
                    'fid_deptt'         => $deptt_name,
                    'fid_tag_no'        => $serial_no,
                    'fid_manual_grn'    => $manual_grn,
                    'fid_purchaseDate'  => $purchaseDate,
                    'fid_date'          => $issuedate,
                    'fid_itemId'        => $itemnameCode,
                    'fid_userId'        => $userInfo['user_id'],
                    ...
                    $sn++;
                        echo '<tr>
                           <th>'.$sn.'</th>
                            <th>'.$qRow->bb_shortname.'-'.$qRow->rm_shortname.'-'.$qRow->itm_shortname.'-'.$qRow->fid_code.'</th>
                            <th>'.$qRow->fid_grn.'</th>
                            <th>'.$qRow->fid_manual_grn.'</th>
                            <th>'.$qRow->itm_name.' ('.$qRow->itm_shortname.')</th>
                            <th>'.$qRow->fid_tag_no.'</th>';
                            
                            $deptt = $this->CRUDModel->get_where_row('department', array('department_id'=> $qRow->fid_deptt));
                            if(!empty($deptt)):
                                echo '<th>'.$deptt->title.'</th>';
                            else:
                                echo '<th></th>';
                            endif;
                            echo '<th>'.$qRow->rm_name.' ('.$qRow->rm_shortname.')</th>
                            <th>'.date('d-m-Y', strtotime($qRow->fid_date)).'</th>    
                            <th>'.$qRow->fid_purchasePrice.'</th>
                            <th>'.date('d-m-Y', strtotime($qRow->fid_purchaseDate)).'</th>
        ...
        $data = array(   
            ...
            'fid_tag_no'         => $isRow->fid_tag_no,
            'fid_manual_grn'        => $isRow->fid_manual_grn,
            'fid_deptt'             => $isRow->fid_deptt,
            'fid_comments'          => $isRow->fid_comments,
           ...
           $query=    $this->InventoryModel->get_items_detailsRow('invt_fixed_item_details',array('fid_id'=>$ifid));
            ...
            $where = array('itm_id'=>$isRow->fid_itemId);
            $q_items = $this->CRUDModel->get_where_row('invt_items',$where);
            $old_quantity = $q_items->item_quantity;
            $total_quantity = $old_quantity - 1;
            $item_data = array( 'item_quantity'=>$total_quantity );
            $this->CRUDModel->update('invt_items',$item_data, $where);
        
            $where1 = array('item_id'=>$isRow->fid_itemId,'ass_id'=>$id);
            $items_data = array( 'remaining_quantity'=>$total_quantity );
...
//            endif;
  ...
  public function inventory_report_search_details(){
            ...
            if($blockNameId):
             ...
             if($roomnameCode):
                $where['fid_roomId'] = $roomnameCode;
                $where2['fid_roomId'] = $roomnameCode;
                $roomnameCode = $this->CRUDModel->get_where_row('invt_rooms',array('rm_id'=>$roomnameCode));
    ...
    public function auto_items_category_consume()
     { 
        $term = trim(strip_tags($_GET['term']));
        
            if( $term == ''){
                
            $result_set             = $this->CRUDModel->get_where_result('invt_items', array('itm_asstId !='=>2));
            $makkah_hotels          = array();
            foreach ($result_set as $row_set) {
                $makkah_hotels[]   = array( 
                    'value'=>$row_set->itm_name.'(Qty = '.$row_set->item_quantity.')',
            ...
            $result_set             = $this->CRUDModel->get_where_like('invt_items', array('itm_asstId !='=>2), $like);
            $labels          = array();
            foreach ($result_set as $row_set) {
            $labels[]        = array( 
                  'value'=>$row_set->itm_name.'(Qty = '.$row_set->item_quantity.')',
...
return false;
    ...
    public function issued_items_change_status()
    {
        ...
        $this->data['item_status']          = $this->CRUDModel->dropDown('invt_items_status', 'Item Status ', 'is_id', 'is_name');
        $this->data['item_issued_status']   = $this->CRUDModel->dropDown('invt_issuance_status', 'Issuance Status ', 'iis_id', 'iis_status');
        $where = "";
        $this->data['emp_idCode'] = "";
        $this->data['blockNameId'] = "";
        $this->data['itemnameCode'] = "";
        $this->data['roomnameCode'] = "";
        $this->data['itemStatusId'] = "";
        $this->data['issuanceStatusId'] = "";
        $this->data['tag_name'] = "";
        if($this->input->post('search')):
            $emp_id         = $this->input->post('emp_idCode');
            $itemnameCode   = $this->input->post('itemnameCode');
            ...
            $roomnameCode   = $this->input->post('roomnameCode');
            $blockNameId    = $this->input->post('blockNameId');
            $item_status_id = $this->input->post('itemStatus');
            $issuance_status_id = $this->input->post('issuanceStatus');
            ...
            if($tagName):
                $where['fid_tag_no'] = $tagName;
                $this->data['tag_name'] = $tagName;
            endif;
            if($roomnameCode):
                $where['fid_roomId'] = $roomnameCode;
            ...
            if($item_status_id):
                $where['invt_items_status.is_id'] = $item_status_id;
                $this->data['itemStatusId'] = $item_status_id;
            endif;
            if($issuance_status_id):
                $where['invt_issuance_status.iis_id'] = $issuance_status_id;
                $this->data['issuanceStatusId'] = $issuance_status_id;
            endif;
    ...
    public function change_status_issuance_item()
    {
        ...
        if($this->input->post('update')):
            ...
            $is_id_old      = $this->input->post('is_id_old');
            $iis_id_old     = $this->input->post('iis_id_old');
            ...
            if($is_id == '1' && $iis_id == '2'):
                if($iis_id_old != $iis_id ):
                    $where = array('itm_id'=>$itm_id);
                    $q_items = $this->CRUDModel->get_where_row('invt_items',$where);
                    $old_quantity = $q_items->item_quantity;
                    $total_quantity = $old_quantity + 1;
                    $item_data = array( 'item_quantity'=>$total_quantity );
...
//                    $this->db->order_by('invt_grn.grn_id','desc');
     ...
     public function item_qty_info(){
        $itemnameCode   = $this->input->post('itemnameCode');
        $where = array('itm_id' => $itemnameCode);
//                    $this->db->join('invt_grn_details','invt_grn_details.grn_id=invt_grn.grn_id');
//                    $this->db->order_by('invt_grn.grn_id','desc');
                    
//        $grn_op =   $this->db->get_where('invt_grn',$where)->row();
        
        $query = $this->CRUDModel->get_where_row('invt_items', $where);
        
        if($query):
        $return['itm_qty'] = $query->item_quantity;
        endif;
         echo json_encode($return);
        
    }
    public function general_inventory_report(){
        ...
        $this->data['page']             = "inventory/reports/general_inventory_report";
        $this->data['page_title']       = 'Inventroy General Report | ECMS';
        $this->data['page_headers']     = 'Inventroy General Report';
        $this->load->view('common/common',$this->data);
    }
    public function inventory_report_search_details_result(){
            ...
            if($emp_id):
                $where['fii_empId'] = $emp_id;
                $where2['fii_empId'] = $emp_id;
                $whereConsum['invt_items_assuance.emp_id'] = $emp_id;
                $empName = $this->CRUDModel->get_where_row('hr_emp_record',array('emp_id'=>$emp_id));
                $details .= '( '.$empName->emp_name.' )';
            endif;
            if($blockNameId):
...
//                $details .= '<strong>'.$bbName->bb_name.'</strong>/ ';
             ...
             if($itemnameCode):
                $where['fid_itemId']    = $itemnameCode;
                $where2['fid_itemId']   = $itemnameCode;
                $whereConsum['item_id'] = $itemnameCode;
                $itemnameCode = $this->CRUDModel->get_where_row('invt_items',array('itm_id'=>$itemnameCode));
                ...
                return false;
                 ...
                 $gcount = "";
                  if($faxItems):
                ?>
                   <h3 class="has-divider text-highlight">Fixed Items Details </h3>
                       <?php echo '<table class="table table-boxed table-hover">
                                <thead>
                                    <tr>
                                  ...
                                  <tbody>';
                                   ...
                                   foreach($faxItems as $fiRow):
                                        $where2['fid_roomId']       = $fiRow->rm_id;
                                        $where2['iis_id']           = '1';
                                        $this->db->select('count(itm_name) as totaItems, itm_name,catm_name,cat_name,at_name');
                                        $this->db->from('invt_fixed_item_details');
                                        $this->db->join('invt_items','invt_items.itm_id=invt_fixed_item_details.fid_itemId');
                                        $this->db->join('invt_fixed_item_issuance','invt_fixed_item_issuance.fii_id=invt_fixed_item_details.fid_fiiId');
                                        $this->db->join('invt_rooms','invt_rooms.rm_id=invt_fixed_item_details.fid_roomId');
                                        ...
                                        $this->db->group_by('itm_id');
                                        $this->db->where($where2);
                $itemsDetailsFixet  =   $this->db->get()->result();
                                      
                                        echo '<tr class="success">
                                            <td></td>  
                                            <td ><string style="font-weight: 900;" >'.$fiRow->rm_name.''.$details.'</strong></td>
                                            <td></td>  
                                            <td></td>  
                                            <td></td>  
                                            <td></td>  
                                          ...
                                          $s_no = '';
                                        foreach($itemsDetailsFixet as $itemRow):
                                            $s_no++;
                                            echo '<tr>';
                                            echo '<td>'.$s_no.'</td>';
                                            echo '<td>'.$itemRow->itm_name.'</td>';
                                            echo '<td>'.$itemRow->catm_name.'</td>';
                                            echo '<td>'.$itemRow->cat_name.'</td>';
                                            echo '<td>'.$itemRow->at_name.'</td>';
                                            echo '<td>'.$itemRow->totaItems.'</td>';
                                            $gcount += $itemRow->totaItems;            
                                            echo '</tr>';
                                        endforeach;
                                ...
                                echo '<tr>';                        
                                 ...
                                 echo '</tbody></table>';
                                          ...
                                          $this->db->join('invt_item_issuance_department','invt_item_issuance_department.iss_dept_id=invt_items_assuance.dept_id');
                                                    $this->db->join('hr_emp_record','invt_items_assuance.emp_id=hr_emp_record.emp_id','left outer');
                                                    $this->db->join('invt_items_assuance_details','invt_items_assuance_details.ass_id=invt_items_assuance.ass_id');
                                                    $this->db->join('invt_items','invt_items.itm_id=invt_items_assuance_details.ass_id');
                                                    $this->db->group_by('dept_name');
                                                    if($whereConsum):
                                                        $this->db->where($whereConsum);
                                                    endif;
                                 $ConsumableItems = $this->db->get_where('invt_items_assuance')->result();
                             $gcountC = '';  
                             if(!empty($whereConsum)):
                    if($ConsumableItems):
                            echo '<h3 class="has-divider text-highlight">Consumable Items Details </h3> 
                            <table class="table table-boxed table-hover">
                                <thead>
                                    <tr>
                                        <th>#</th>
...
//                                     $whereConsum = '';
                                     $gcountC = '';
                                     foreach($ConsumableItems as $cRow):
                                                $whereConsum['dept_id']= $cRow->dept_id;
                                                $this->db->select('invt_items_assuance.ass_id,sum(quantity) as Tquantity, itm_name,catm_name,cat_name,at_name');
                                                $this->db->join('invt_items_assuance_details','invt_items_assuance_details.ass_id=invt_items_assuance.ass_id');
                                 ...
                                 $ConsumableItemsDetails = $this->db->get_where('invt_items_assuance',$whereConsum)->result();

                                 echo '<tr class="success">
                                                   <td></td>  
                                                   <td ><string style="font-weight: 900;" >'.$cRow->dept_name.' '.$details.'</strong></td>
                                                   <td></td>  
                                                   <td></td>  
                                                   <td></td>  
                                                   <td></td>  
                                               </tr>';
                                 if($ConsumableItemsDetails ):
                                     
                                     
                                     
                                     $s_no1 = '';
                                     foreach($ConsumableItemsDetails as $ComItemDetailsRows):
                                            $s_no1++;
                                            echo '<tr>';
                                            echo '<td>'.$s_no1.'</td>';
                                             echo '<td>'.$ComItemDetailsRows->itm_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->catm_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->cat_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->at_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->Tquantity.'</td>';
                                                $gcountC += $ComItemDetailsRows->Tquantity;           
                                            echo '</tr>';
                                        endforeach; 
     ...
     public function inventory_report_search_details_result_item_wise(){
                                ...
                                <thead>
                                  ...
                                  <tbody>';
                                   ...
                                   foreach($faxItems as $fiRow):
                                        $where2['fid_roomId']       = $fiRow->rm_id;
                                        $where2['iis_id']           = '1';
                                        $this->db->select('itm_name,catm_name,cat_name,at_name,emp_name,invt_fixed_item_details.fid_tag_no,department.title as deptt_name');
...
//                                        $this->db->group_by('itm_id');
                                        $this->db->where($where2);
                                $itemsDetailsFixet  =   $this->db->get('invt_fixed_item_details')->result();
                                      
                                        echo '<tr class="success">
                                             <td></td> 
                                            <td ><string style="font-weight: 900;" >'.$fiRow->rm_name.'</strong></td>
                                        ...
                                        foreach($itemsDetailsFixet as $itemRow):
                                            ...
                                            echo '<td>'.$s_no.'</td>';
                                            echo '<td>'.$itemRow->itm_name.'</td>';
                                            echo '<td>'.$itemRow->fid_tag_no.'</td>';
                                            echo '<td>'.$itemRow->emp_name.'</td>';
                                            echo '<td>'.$itemRow->deptt_name.'</td>';
                                            echo '<td>'.$itemRow->catm_name.'</td>';
                                            echo '<td>'.$itemRow->cat_name.'</td>';
                                            echo '<td>'.$itemRow->at_name.'</td>';
...
//                                     $whereConsum = '';
                                     ...
                                     foreach($ConsumableItems as $cRow):
                                                $whereConsum['dept_id']= $cRow->dept_id;
                                                $this->db->select('itm_name,catm_name,cat_name,at_name,emp_name');
                                                $this->db->join('invt_items_assuance_details','invt_items_assuance_details.ass_id=invt_items_assuance.ass_id');
                                                $this->db->join('hr_emp_record','hr_emp_record.emp_id=invt_items_assuance.emp_id');
                                                $this->db->join('invt_items','invt_items.itm_id=invt_items_assuance_details.item_id');
                                                $this->db->join('invt_main_category','invt_main_category.catm_id=invt_items.catm_id', 'left outer');
                                                $this->db->join('invt_category','invt_category.cat_id=invt_items.itm_catId', 'left outer');
                                                $this->db->join('invt_assets_type','invt_assets_type.at_id=invt_items.itm_asstId', 'left outer');
//                                                $this->db->group_by('itm_name');
                                 ...
                                 echo '<tr class="success">
                                                   <td></td>  
                                                   <td ><string style="font-weight: 900;" >'.$cRow->dept_name.'</strong></td>
                                                   <td></td>  
                                                   <td></td>  
                                                   <td></td>  
                                                   <td></td>  
                                 ...
                                 if($ConsumableItemsDetails ):
                                     ...
                                     foreach($ConsumableItemsDetails as $ComItemDetailsRows):
                                            ...
                                            echo '<td>'.$s_no1.'</td>';
                                             echo '<td>'.$ComItemDetailsRows->itm_name.'</td>';
                                             echo '<td>'.$ComItemDetailsRows->emp_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->catm_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->cat_name.'</td>';
                                            echo '<td>'.$ComItemDetailsRows->at_name.'</td>';
                                            echo '</tr>';
                                  ...
                                  echo '<tr>';                        
                                        ...
                                        echo '<td></td>';
                                        echo '<td></td>';
                                        echo '<td><string style="font-weight: 900;" >Total</strong></td>';
                                        echo '<td><string style="font-weight: 900;" >'.$gcountC.'</strong> </td>';
                                        echo '</tr>';          
                                 echo '</tbody></table>';
    ...
    public function print_issued_items()
    {
        $fid_id = $this->uri->segment(3);
        $wheredata = array('invt_fixed_item_details.fid_id'=>$fid_id);
        $this->data['fid_result'] = $this->InventoryModel->get_issued_item_data('invt_fixed_item_details', $wheredata);
//        echo '<pre>'; print_r($this->data['fid_result']); die;
        $this->data['page_title']   = 'Print Issued Items | ECMS';
        $this->data['page']         = 'inventory/print_issued_items';
        $this->load->view('common/common',$this->data); 
    }

    public function quotations(){

        $this->data['page']         = "inventory/quotations/quotations";
        $this->data['fixedItems']   = $this->InventoryModel->get_items_lists();
         $this->data['page_header'] = 'Add Quotations';
        $this->data['page_title']   = 'Quotations | ECMS';
        $this->load->view('common/common',$this->data);   
    ...
    public function add_quotation_item(){
        ...
        $quote_title    = $this->input->post('qTitle');
        $quote_date     = $this->input->post('qDate');
        $item_name      = $this->input->post('itemName');
        $item_code      = $this->input->post('itemCode');
        $hidden_id      = $this->input->post('hiddenId');
        $where_array = array(
            'quot_title'    => $quote_title,
            'quot_date'     => date('Y-m-d', strtotime($quote_date))
        );
        $check_title = $this->CRUDModel->get_where_row('invt_quotations', $where_array);
//        echo '<pre>'; print_r($check_title); die;
        if(empty($hidden_id)):
            ...
            if(empty($check_title)):
                $quote_array = array(
                    'quot_title'    => $quote_title,
                    'quot_date'     => date('Y-m-d', strtotime($quote_date)),
                    'quot_user_id'  => $this->userInfo->user_id,
                    'quot_timestamp' => date('Y-m-d H:i:s')
                );

                if(empty($hidden_id)):
                    $id = $this->CRUDModel->insert('invt_quotations', $quote_array);
                else:
                    $id = $hidden_id;
                endif;

                $check_item = $this->CRUDModel->get_where_row('invt_quotation_items', array('item_id'=>$item_code, 'q_id'=>$id));
                if($check_item):
                    $return_json['item'] = false;
                    $return_json['msg'] = 'Item already exist';
                else:
                    $itm_demo_array = array(
                        'q_id'      => $id,
                        'item_id'   => $item_code
                        );
                    $this->CRUDModel->insert('invt_quotation_items', $itm_demo_array);
                    $return_json['item'] = true;
            ...
            $return_json['qid'] = $id;
              ...
              $return_json['title'] = false;  
              $return_json['msg'] = 'Entry already exist.';  
            endif;
    ...
    public function demo_quotation_item(){
        
        $qid = $this->input->post('quotID');
        $query = $this->InventoryModel->get_demo_items('invt_quotation_items', array('q_id' => $qid));
        ...
        echo '<div class="table-responsive">                      
                <table class="table table-hover">
                    ...
                    foreach($query as $qRow):
                        ...
                        echo '<tr>
                           <th>'.$sn.'</th>
                            <th>'.$qRow->itm_name.'<input type="hidden" name="qId" id="qId" value="'.$qid.'"></th>
                            <th><a href="JavaScript:Void(0)" class="DeleteItem" id="'.$qRow->id.'"><i class="fa fa-trash"></i></a></th>
                        </tr>'; 
                    endforeach;
                    echo '</tbody>
                </table>
            ...
            jQuery(document).ready(function(){
                jQuery('.DeleteItem').on('click',function(){
                    ...
                    jQuery.ajax({
                        ...
                        data   : data,
                        success :function(result){
                            var newdata = { 'quotID' : jQuery('#qId').val()};
                            jQuery.ajax({
                                type : 'post',
                                url : 'DemoQuotationItem',
                                data : newdata,
                                success : function(result){
                                    jQuery('#showQuotationItems').html(result);
                                }
                            });
                        }
                    });
    ...
    public function delete_quotation_item(){
        $this->CRUDModel->deleteid('invt_quotation_items', array('id' => $this->input->post('entryId')));
    }
    ...
    public function save_quotation_items(){
        ...
        //Get Result form Demo Table
        $demoDetails =  $this->CRUDModel->get_where_result('invt_quotation_items', array('q_id' => $quote_id));
  
        if($demoDetails):    
            $return_json['save'] = true;
            $return_json['qid'] = $quote_id;
        else:
            $return_json['save'] = false;
        endif;
        echo json_encode($return_json);
    }
    
    public function quotation_items_grid(){
        
        $qid = $this->input->post('quoteId');
        $query = $this->InventoryModel->get_qd_items('invt_quotation_items', array('q_id' => $qid));
        
        echo form_open('',array('id'=>'quotation_form'));
        echo '<div class="table-responsive">                      
                <table class="table table-hover">
                    <thead>
                        <tr>
                    ...
                    foreach($query as $qRow):
                        ...
                        echo '<tr id="'.$qRow->item_id.'">
                           <th>'.$sn.'</th>
                            ...
                            <th><input type="text" name="q_spec[]" id="spec'.$qRow->item_id.'" class="q_spec"></th>
             ...
             <div class="row"> 
                ...
                echo '</div>
                <div class="col-md-4 col-sm-5">
                    <button type="button" class="btn btn-theme"  id="saveQuotation"><i class="fa fa-plus"></i>Add Quoation</button>
            ...
            jQuery(document).ready(function(){
                ...
                jQuery('#saveQuotation').on('click', function(){
                    ...
                    jQuery.ajax({
                        ...
                        datatype: 'json',
                        data    : jQuery('#quotation_form').serialize(),
                        success : function(result){
    ...
    public function save_quotation_record(){
        
        $quote_id       = $this->input->post('quoteId');
        $supplier_id    = $this->input->post('supplierId');
        $item_id        = $this->input->post('q_itm_id');
        $itm_specs      = $this->input->post('q_spec');
        $quantity       = $this->input->post('q_qty');
        $unit_price     = $this->input->post('q_unit');
        $total_amount   = $this->input->post('q_total');
//        echo '<pre>'; print_r($this->input->post()); die;
        $result_array = array();
        $check = $this->CRUDModel->get_where_result('invt_quotations_detail', array('qd_quot_id' => $quote_id, 'qd_supplier_id' => $supplier_id));
        if($check):
            $return_json['save'] = false;
        else:
            foreach($item_id as $row => $key):
                $result_array = array(
                    'qd_quot_id'        => $quote_id,
                    'qd_supplier_id'    => $supplier_id,
                    'qd_product_id'     => $item_id[$row],
                    'qd_specs'          => $itm_specs[$row],
                    'qd_product_qty'    => $quantity[$row],
                    'qd_product_price'  => $unit_price[$row],
                    'qd_total_price'    => $total_amount[$row],
                    'qd_datetime'       => date('Y-m-d H:i:s'),
                    'qd_user_id'        => $this->userInfo->user_id,
                );
                $this->CRUDModel->insert('invt_quotations_detail', $result_array);
            endforeach;
        endif;
        echo json_encode($return_json);
    }
 
    public function quotation_grid(){
        
        $q_id = $this->input->post('quotation_id');
        $query_t = $this->InventoryModel->get_quote('invt_quotations_detail', array('quot_id' => $q_id));
//        echo '<pre>'; print_r($this->input->post()); die;
        if($query_t):
            $numb = '';
            echo '<div class="table-responsive">';
                ...
                echo '<table class="table table-hover" id="table">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2">Supplier: '.$wrow->supplier_name.'</th>
                            <th>
                                <button class="btn-small btn-theme editQuot" id="'.$wrow->qd_supplier_id.','.$q_id.'" data-toggle="modal" data-target="#myModal">Edit</button>
                                <button class="btn-small btn-danger deleteQuote" id="'.$wrow->qd_supplier_id.','.$q_id.'">Delete</button>
                                <input type="hidden" name="qu_id" id="qu_id" value="'.$q_id.'">
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    $query = $this->InventoryModel->get_qd_details('invt_quotations_detail', array('qd_quot_id' => $q_id, 'qd_supplier_id' => $wrow->qd_supplier_id));
        ...
        ?><script>
            jQuery(document).ready(function(){
                jQuery('.deleteQuote').on('click', function(){
                    var r = confirm('Are you sure?');
                    if(r === true){
                        var arr_id = $(this).attr('id');
                        jQuery.ajax({
                            type : 'post',
                            url : 'DeleteQuotationDetails',
                            data : { 'arr_id' : arr_id },
                            success : function(){
                                jQuery.ajax({
                                type : 'post',
                                url : 'QuotationsGrid',
                                data : { 'quotation_id' : jQuery('#qu_id').val()},
                                success : function(response){
                                    jQuery('#QuotationGrid').html(response);
                                }
                            });
                ...
                jQuery('.editQuot').on('click', function(){
                    var suppId = $(this).attr('id');
                    jQuery.ajax({
                        type : 'post',
                        url : 'EditQuotationDetails',
                        data : { 
                            'suppId' : suppId
                        },
                        success : function(response){
                             jQuery('#quoteDetails').html(response);
                        }
                    }); 
                });
            });
    ...
    public function edit_quotation_details(){
        ...
        $ids = explode(',', $this->input->post('suppId'));
        $supplier = $ids[0];
        $quot_id = $ids[1];
        $query = $this->InventoryModel->get_qd_details('invt_quotations_detail', array('qd_quot_id' => $quot_id, 'qd_supplier_id' => $supplier));
        
        echo form_open('',array('id'=>'update_quotation_form'));
        echo '<!-- Modal content-->
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                  <h4 class="modal-title">Update Quotation Details</h4>
                </div>
            ...
            <div class="table-responsive">                      
                <table class="table table-hover">
                    ...
                    foreach($query as $qRow):
                        $sn++;
                        echo '<tr id="'.$qRow->itm_id.'">
                           <th>'.$sn.'</th>
                            <th>'.$qRow->item_name.'<input type="hidden" name="q_itm_id[]" id="qo_itm_id" value="'.$qRow->itm_id.'"></th>
                            <th><input type="text" value="'.$qRow->qd_specs.'" name="qo_spec[]" id="speco'.$qRow->itm_id.'" class="qo_spec" style="width:100px"></th>
                            <th><input type="number" value="'.$qRow->qd_product_qty.'" name="qo_qty[]" id="qtyo'.$qRow->itm_id.'" class="qo_qty" style="width:100px"></th>
                            <th><input type="number" value="'.$qRow->qd_product_price.'" name="qo_unit[]" id="unito'.$qRow->itm_id.'" class="qo_unit" style="width:100px"></th>
                            <th><input type="number" value="'.$qRow->qd_total_price.'" name="qo_total[]" id="totalo'.$qRow->itm_id.'" class="qo_total" readonly style="width:100px"></th>
                                <input type="hidden" name="qod_id[]" id="qod_id" value="'.$qRow->qd_id.'">
                        </tr>'; 
                    endforeach;
                    echo '</tbody>
                </table>
                ...
                <button type="button" class="btn btn-theme"  id="saveEditQuot"><i class="fa fa-save"></i>Update</button>
                  <button type="button" class="btn btn-default" data-dismiss="modal" id="modalClose">Close</button>
                 <input type="hidden" id="Quote" value="'.$quot_id.'">
                </div>
              </div>';
            echo form_close();
            ...
            jQuery(document).ready(function(){
                ...
                jQuery('.qo_qty').on('change', function(){
                    var rowId   = $(this).parent().parent().attr('id');
                    var qty     = jQuery(this).val();
                    var unit    = jQuery('#unito'+rowId).val();
                    var total = isNaN(parseInt(qty) * parseInt(unit)) ? 0 :(qty * unit);
                    jQuery('#totalo'+rowId).val(total);
                });
                
                jQuery('.qo_unit').on('change', function(){
                    var rowId   = $(this).parent().parent().attr('id');
                    var qty     = jQuery(this).val();
                    var unit    = jQuery('#qtyo'+rowId).val();
                    var total = isNaN(parseInt(qty) * parseInt(unit)) ? 0 :(qty * unit);
                    jQuery('#totalo'+rowId).val(total);
                });
                
                
                jQuery('#saveEditQuot').on('click', function(){
                    $("#table_body tr").each(function() {
                        if(jQuery('#qo_qty'+this.id).val() === ''){
                            jQuery('#qo_qty'+this.id).focus();
                            alert('Please add item details');
                            return false;
                        }
                        if(jQuery('#unito'+this.id).val() === ''){
                            jQuery('#unito'+this.id).focus();
                            alert('Please add item details');
                            return false;
                        }
                        });
                    jQuery.ajax({
                        type    :'post',
                        url     :'UpdateQuotationDetails',
                        datatype: 'json',
                        data    : jQuery('#update_quotation_form').serialize(),
                        success : function(result){
                            var quotation_id = jQuery('#Quote').val();
    ...
    public function update_quotation_details(){
        
        $rec_id         = $this->input->post('qod_id');
        $item_id        = $this->input->post('qo_itm_id');
        $item_specs     = $this->input->post('qo_spec');
        $quantity       = $this->input->post('qo_qty');
        $unit_price     = $this->input->post('qo_unit');
        $total_amount   = $this->input->post('qo_total');
//        echo '<pre>'; print_r($this->input->post()); die;
        $result_array = array();
        
            foreach($rec_id as $row => $key):
                $result_array = array(
                    'qd_specs'          => $item_specs[$row],
                    'qd_product_qty'    => $quantity[$row],
                    'qd_product_price'  => $unit_price[$row],
                    'qd_total_price'    => $total_amount[$row],
                    'qd_update_datetime' => date('Y-m-d H:i:s'),
                    'qd_update_by'      => $this->userInfo->user_id,
                );
                $where_arr = array(
                    'qd_id' => $rec_id[$row]
                );
                $this->CRUDModel->update('invt_quotations_detail', $result_array, $where_arr);
            endforeach;
//            
//        echo json_encode($return_json);
    }
    ...
    public function delete_quotation_details(){

        $arr_id = explode(',', $this->input->post('arr_id'));
        $supplier   = $arr_id[0];
        $quot_id    = $arr_id[1];
        
        $this->CRUDModel->deleteid('invt_quotations_detail', array('qd_supplier_id'=>$supplier, 'qd_quot_id'=>$quot_id));
        
    }
    
    public function quotations_record(){

        $this->data['page']         = "inventory/quotations/quotations_record";
        $this->data['fixedItems']   = $this->InventoryModel->get_items_lists();
         $this->data['page_header'] = 'All Quotations Record';
        $this->data['page_title']   = 'Quotations Record | ECMS';
        $this->load->view('common/common',$this->data);   
        
    }
    
    public function search_quotations(){

        $title  = $this->input->post('quot_title');
        $s_date = $this->input->post('q_start_date');
        $e_date = $this->input->post('q_end_date');
        
        $like   = '';
        if(!empty($title)):
                $like['invt_quotations.quot_title'] = $title;
                $this->data['qTitle'] = $title;
        endif;
        
        $query_t = $this->InventoryModel->get_quote_record('invt_quotations', $s_date, $e_date, $like);
        if($query_t):
            foreach($query_t as $wrow):
                echo '<div class="panel panel-theme">
                    <div class="panel-body">
                        <div class="table-responsive">
                            ...
                            <thead>
                                <tr>
                                    <th colspan="4" align="center" style="font-size: 16px;">Title: '.$wrow->quot_title.'</th>
                                    <th>Date: '.date('d-m-Y', strtotime($wrow->quot_date)).'</th>
                                    <th colspan="2">';
                                        $check_cs = $this->CRUDModel->get_where_row('invt_comparative_statement', array('cs_quote_id'=>$wrow->quot_id));
                                        if($check_cs):
                                            echo '<a class="btn btn-danger disabled btn-sm" href="">Comparative</a> 
                                            <a class="btn btn-theme disabled btn-sm" href="">Edit</a>  
                                            <a class="btn btn-primary btn-sm" href="GenerateComparative/'.$check_cs->cs_id.'">Print</a> ';
                                        else: 
                                            echo '<a class="btn btn-danger btn-sm" href="ComparativeStatement/'.$wrow->quot_id.'">Comparative</a> 
                                            <a class="btn btn-theme btn-sm" href="AddMoreQuotations/'.$wrow->quot_id.'">Edit</a>  
                                            <a class="btn btn-primary btn-sm disabled" href="">Print</a> ';
                                        endif;
                                    echo '</th>
                                </tr>
                            </thead>';

                            $query_s = $this->InventoryModel->get_quote_supplier('invt_quotations_detail', array('qd_quot_id' => $wrow->quot_id));
                            if($query_s):
                                $numb = '';
                                foreach($query_s as $srow):
                                $numb++;
                                echo '<thead>
                                    <tr style="background-color: #eee;">
                                        <th align="center">'.$numb.'</th>
                                        <th colspan="6">'.$srow->supplier_fname.'</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                ...
                                $query = $this->InventoryModel->get_qd_details('invt_quotations_detail', array('qd_quot_id' => $wrow->quot_id, 'qd_supplier_id' => $srow->supplier_fid));
                                foreach($query as $crow):
                                    $serial++;
                                    echo '<tr>
                                        <td></td>
    ...
    public function quotation_add_more(){

        $qid = $this->uri->segment(2);
        
        $this->data['qid']  = $qid;
        $this->data['query'] = $this->InventoryModel->get_qd_items('invt_quotation_items', array('q_id' => $qid));
        
        $this->data['page']         = "inventory/quotations/quotations_add";
         $this->data['page_header'] = 'Add Quotations';
        $this->data['page_title']   = 'Add Quotations | ECMS';
        $this->load->view('common/common',$this->data);   
        
    }
    
    public function comparative_statement(){

        $qid = $this->uri->segment(2);
        $this->data['quotid']   = $qid;
        $this->data['query']    = $this->InventoryModel->get_quote('invt_quotations_detail', array('quot_id' => $qid));
        
        $this->data['employee_id']  = '';
        $this->data['employees']      = $this->CRUDModel->dropDownEmpScale('hr_emp_record', 'Select', 'emp_id', 'emp_name', array('hr_emp_scale.title >=' => 17, 'hr_emp_record.emp_status_id' => 1));
        
        $this->data['page']         = "inventory/quotations/comparative_statement";
        $this->data['page_header']  = 'Comaparative Statement';
        $this->data['page_title']   = 'Comaparative Statement | ECMS';
        $this->load->view('common/common',$this->data);   
        
    }
    
    public function save_comparative_statement(){

        $quot_id        = $this->input->post('quotation_id');
        $fy_id          = $this->input->post('financial_year');
        $minute_sheet   = $this->input->post('min_sheet');
        $requisition    = $this->input->post('req_no');
        $cs_date        = $this->input->post('cs_date');
        $signature      = $this->input->post('signature');
        $comp_for       = $this->input->post('quote_for');
        
        if($this->input->post('submit')):
            $acctOff    = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'23', 'emp_status_id'=>'1'));
            $adminOff   = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'5', 'emp_status_id'=>'1'));
            $dirFince   = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'4', 'emp_status_id'=>'1'));
//            $vpAdmin    = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'72', 'emp_status_id'=>'1'));
            $save_data = array(
                'cs_quote_id'       => $quot_id,
                'cs_for'            => $comp_for,
                'cs_financial_year' => $fy_id,
                'cs_minute_sheet_no' => $minute_sheet,
                'cs_requisistion_no' => $requisition,
                'cs_acct_off_id'    => $acctOff->emp_id,
                'cs_est_mngr_id'    => $signature,
                'cs_admin_off_id'   => $adminOff->emp_id,
                'cs_dir_finance_id' => $dirFince->emp_id,
//                'cs_vp_admin_id'    => $vpAdmin->emp_id,
                'cs_vp_admin_id'    => 32, // Naveed Ali
                'cs_date'           => date('Y-m-d', strtotime($cs_date)),
                'cs_timestamp'      => date('Y-m-d H:i:s'),
                'cs_user_id'        => $this->userInfo->user_id,
                );
            $csId = $this->CRUDModel->insert('invt_comparative_statement', $save_data);
            
            $order_id  = $this->InventoryModel->get_cs_quote_data(array('quot_id'=>$quot_id));
//            echo '<pre>'; print_r($order_id); die;
            $sn = '';
            foreach($order_id as $orow):
                $sn++;
                $set = array(
                    'comparative_order' => $sn,
                );
                $where = array(
                    'qd_quot_id' => $orow->qd_quot_id,
                    'qd_supplier_id' => $orow->qd_supplier_id,
                );
                $this->CRUDModel->update('invt_quotations_detail', $set, $where);
            endforeach;
            
            redirect('GenerateComparative/'.$csId);
        endif;
//        echo '<pre>'; print_r($this->input->post()); die;
    }
    
    public function generate_comparative_statement(){

    $com_id = $this->uri->segment(2);
    
    $this->data['cs_data']  = $this->InventoryModel->get_comparative_data('invt_comparative_statement', array('cs_id'=>$com_id));
    $this->data['qd_data']  = $this->InventoryModel->get_cs_quotations_record(array('qd_quot_id'=>$this->data['cs_data']->cs_quote_id));
    $this->data['itm_data'] = $this->InventoryModel->get_cs_item_data(array('qd_quot_id'=>$this->data['cs_data']->cs_quote_id));
    $this->data['u_data']   = $this->InventoryModel->get_u_data(array('id'=>$this->userInfo->user_id));
    
    $this->data['page']         = "inventory/quotations/show_comparative_statement";
    $this->data['page_header']  = 'Comaparative Statement';
    $this->data['page_title']   = 'Comaparative Statement | ECMS';
    $this->load->view('common/common',$this->data);
    }
    
    public function comparative_state_record(){

        $this->data['page']         = "inventory/quotations/comparative_record";
        $this->data['fixedItems']   = $this->InventoryModel->get_items_lists();
         $this->data['page_header'] = 'Comparative Statements Record';
        $this->data['page_title']   = 'Comparative Statements Record | ECMS';
        $this->load->view('common/common',$this->data);   
        
    }
    
    public function search_comparative_statement(){

        $title          = $this->input->post('cs_title');
        $min_sheet      = $this->input->post('minute_sheet');
        ...
        if(!empty($min_sheet)):
                $where['invt_comparative_statement.cs_minute_sheet_no'] = $min_sheet;
                $this->data['min_sheet_no'] = $min_sheet;
        endif;
        
        if(!empty($requisition)):
                $where['invt_comparative_statement.cs_requisistion_no'] = $requisition;
                $this->data['requisition_no'] = $requisition;
        endif;
        
        $query_t = $this->InventoryModel->get_comparative_record('invt_comparative_statement', $s_date, $e_date, $like, $where);
        if($query_t):
            foreach($query_t as $wrow):
                echo '<div class="panel panel-theme">
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-hover" id="table">
                                <thead>
                                    <tr>
                                        <th colspan="3" align="center" style="font-size: 16px;">Title: '.$wrow->quot_title.'</th>
                                        <th>Minute Sheet # '.$wrow->cs_minute_sheet_no.'</th>
                                        <th>Requisition # '.$wrow->cs_requisistion_no.'</th>
                                        <th>Date: '.date('d-m-Y', strtotime($wrow->cs_date)).'</th>
                                        <th style="text-align: right"><a class="btn btn-primary btn-sm" href="GenerateComparative/'.$wrow->cs_quote_id.'"><i class="fa fa-print"></i>Print Statement</a></th>
                                    </tr>
                                </thead>
                            </table>
                            <table class="table table-hover" id="table">';

                            $query_s = $this->InventoryModel->get_cs_supplier('invt_quotations_detail', array('qd_quot_id' => $wrow->quot_id));
                            if($query_s):
                                $numb = '';
                                foreach($query_s as $srow):
                                $numb++;
                                echo '<thead>
                                    <tr style="background-color: #eee;">
                                        <th align="center">'.$numb.'</th>
                                        <th colspan="5">'.$srow->supplier_fname.'</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
    ...
    public function comparative_state_ps(){

        $this->data['page']         = "inventory/purchaseOrder/comparative_state_record";
        $this->data['fixedItems']   = $this->InventoryModel->get_items_lists();
         $this->data['page_header'] = 'Search Comparative Statements For Purchase Order';
        $this->data['page_title']   = 'Comparative Statement | ECMS';
        $this->load->view('common/common',$this->data);   
        
    }
    
    public function search_cs_for_po(){
        ...
        $like   = '';
        $where['invt_comparative_statement.cs_for']  = 1;
        ...
        if($query_t):
            $serial = '';
                echo '<div class="panel panel-theme">
                        <div class="table-responsive">
                            <table  class="table table-boxed table-hover table-bordered" id="table">
                                <thead>
                                    <tr>
                                        <th>S No.</th>
                                        <th>Title</th>
                                        <th>Items</th>
                                        <th>Minute Sheet No.</th>
                                        <th>Requisition No.</th>
                                        <th>Comparative Date</th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>';
                                foreach($query_t as $wrow):
                                    ...
                                    echo '<tr>
                                        <td>'.$serial.'</td>
                                        <td>'.$wrow->quot_title.'</td>
                                        <td>';
                                            $query_itm = $this->InventoryModel->get_qd_items('invt_quotation_items', array('q_id' => $wrow->quot_id));
                                            if($query_itm):
                                                $isn = '';
                                                foreach($query_itm as $irow):
                                                    $isn++;
                                                    echo '<strong style="color: green">'.$isn.')</strong> '.$irow->itm_name.'&nbsp;&nbsp;&nbsp; ';
                                                endforeach;
                                            endif;
                                        echo '</td>
                                        <td>'.$wrow->cs_minute_sheet_no.'</td>
                                        <td>'.$wrow->cs_requisistion_no.'</td>
                                        <td>'.date('d-m-Y', strtotime($wrow->cs_date)).'</td>
                                        <td><a class="btn btn-primary btn-sm" href="PurchaseOrderNew/'.$wrow->cs_id.'/'.$wrow->quot_id.'"><i class="fa fa-plus"></i>Add Purchse Order</a></td>
                                    </tr>';
                                endforeach;        
                                echo '</tbody>
                            </table>
    ...
    {
        $csId = $this->uri->segment(2);
        $qtId = $this->uri->segment(3);
        
        $this->data['quoteId']  = $qtId;
        $this->data['csId']     = $csId;
        $this->data['qd_supplier_id'] = '';
        $this->data['supplier']     = $this->CRUDModel->dropDownSupplier('invt_quotations_detail', '', 'qd_supplier_id', 'sp_name', array('invt_quotations_detail.qd_quot_id'=>$qtId));
        
        $this->data['adminOff']   = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'5', 'emp_status_id'=>'1'));
        $this->data['superdnt']   = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'8', 'emp_status_id'=>'1', 'department_id'=>'3'));
        
        $this->data['page_title']   = 'Purchase Order | ECMS';
        $this->data['page']         = 'inventory/purchaseOrder/new_purchase_order';
        $this->load->view('common/common',$this->data); 
    }
    
    public function new_purchase_order_items(){
        
        $qid = $this->input->post('qtid');
        $sid = $this->input->post('posp');
//        echo '<pre>'; print_r($this->input->post()); die;
        $query = $this->InventoryModel->get_qd_details('invt_quotations_detail', array('qd_quot_id' => $qid, 'qd_supplier_id' => $sid));
//        echo '<pre>'; print_r($query); die;
        echo '<div>                      
                <table class="table table-hover">
                    ...
                    foreach($query as $qRow):
                        echo '<tr id="'.$qRow->itm_id.'">
                            <th>'.$qRow->item_name.'<input type="hidden" name="q_itm_id[]" id="q_itm_id" value="'.$qRow->itm_id.'"></th>
                            <th><input type="text" name="q_brand[]" id="brand'.$qRow->itm_id.'" class="q_brand" value="'.$qRow->qd_specs.'"></th>
                            <th><input type="number" name="q_qty[]" id="qty'.$qRow->itm_id.'" class="q_qty" value="'.$qRow->qd_product_qty.'"></th>
                            <th><input type="number" name="q_unit[]" id="unit'.$qRow->itm_id.'" class="q_unit" readonly value="'.$qRow->qd_product_price.'"></th>
                            <th><input type="number" name="q_total[]" id="total'.$qRow->itm_id.'" class="q_total" readonly value="'.$qRow->qd_total_price.'"></th>
                            <th><input type="button" name=""  id="'.$qRow->itm_id.'" class="btn btn-danger btn-xs remove_row" value="Delete"></th>
    ...
    public function save_purchaseorder_record(){
        
        $dirFince   = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'4', 'emp_status_id'=>'1'));
        $vpAdmin    = $this->InventoryModel->get_by_id_row('hr_emp_record', array('current_designation'=>'72', 'emp_status_id'=>'1'));
        
        $data            = array(
            'comparative_id'    => $this->input->post('cs_id'),
            'year'              => $this->input->post('year'),
            'order_date'        => date('Y-m-d', strtotime($this->input->post('order_date'))),
            'issued_by'         => $this->input->post('issued_by'),
            'issued_to'         => $this->input->post('po_supplier'),
            'ship_to'           => $this->input->post('ship_to'),
            'prepared_by'       => $this->input->post('prepared_by'),
            'authorized_by'     => $this->input->post('authorized_by'),
//            'vp_admin'          => $vpAdmin->emp_id,
            'vp_admin'          => 32,
            'director_finance'  => $dirFince->emp_id,
            'create_by'         => $this->userInfo->user_id,
            'create_datetime'   => date('Y-m-d H:i:s'),
            );   
        $porderId = $this->CRUDModel->insert('invt_purchase_order', $data);
        $item_id        = $this->input->post('q_itm_id');
        $brand          = $this->input->post('q_brand');
        $quantity       = $this->input->post('q_qty');
        $unit_price     = $this->input->post('q_unit');
        $total_amount   = $this->input->post('q_total');
        ...
        foreach($item_id as $row => $key):
            $result_array = array(
                ...
                'price'         => $unit_price[$row],
                'total_price'   => $total_amount[$row],
                'date'          => date('Y-m-d'),
                'user_id'       => $this->userInfo->user_id,
            );
            $this->CRUDModel->insert('invt_purchase_order_details', $result_array);
        endforeach;
    }
 
    public function comparative_state_wko(){

        $this->data['page']         = "inventory/workOrder/comparative_state_record";
        $this->data['fixedItems']   = $this->InventoryModel->get_items_lists();
         $this->data['page_header'] = 'Search Comparative Statements For Work Order';
        $this->data['page_title']   = 'Comparative Statement | ECMS';
        $this->load->view('common/common',$this->data);   
        
    }
    
    public function search_cs_for_wko(){
        ...
        $like   = '';
        $where['invt_comparative_statement.cs_for']  = 2;
                            ...
                            <table  class="table table-boxed table-hover table-bordered" id="table">
                                ...
                                foreach($query_t as $wrow):
                                    ...
                                    echo '<tr>
                                        ...
                                        <td>'.$wrow->cs_requisistion_no.'</td>
                                        <td>'.date('d-m-Y', strtotime($wrow->cs_date)).'</td>
                                        <td><a class="btn btn-primary btn-sm" href="WorkOrderNew/'.$wrow->cs_id.'/'.$wrow->quot_id.'"><i class="fa fa-plus"></i>Add Work Order</a></td>
                                    </tr>';
                                endforeach;
                                echo '</tbody>
                            </table>
    ...
    public function new_work_order()
    {
        ...
        $this->data['page_title']   = 'Work Order | ECMS';
        $this->data['page']         = 'inventory/workOrder/new_work_order';
        $this->load->view('common/common',$this->data); 
    }
    
    public function new_work_order_items(){
...
//        echo '<pre>'; print_r($query); die;
        echo '<div>                      
                <table class="table table-hover">
                    ...
                    foreach($query as $qRow):
                        echo '<tr id="'.$qRow->itm_id.'">
                            <th>'.$qRow->item_name.'<input type="hidden" name="q_itm_id[]" id="q_itm_id" value="'.$qRow->itm_id.'"></th>
                            <th><input type="number" name="q_days[]" id="days'.$qRow->itm_id.'" class="q_days" value=""></th>
                            <th><input type="number" name="q_qty[]" id="qty'.$qRow->itm_id.'" class="q_qty" value="'.$qRow->qd_product_qty.'"></th>
                            <th><input type="number" name="q_unit[]" id="unit'.$qRow->itm_id.'" class="q_unit" readonly value="'.$qRow->qd_product_price.'"></th>
                            <th><input type="number" name="q_total[]" id="total'.$qRow->itm_id.'" class="q_total" readonly value="'.$qRow->qd_total_price.'"></th>
                            <th><input type="button" name=""  id="'.$qRow->itm_id.'" class="btn btn-danger btn-xs remove_row" value="Delete"></th>
    ...
    public function save_workorder_record(){
        ...
        $worderId = $this->CRUDModel->insert('invt_work_order', $data);
        $item_id        = $this->input->post('q_itm_id');
        $days          = $this->input->post('q_days');
        $quantity       = $this->input->post('q_qty');
        $unit_price     = $this->input->post('q_unit');
        $total_amount   = $this->input->post('q_total');
        ...
        foreach($item_id as $row => $key):
            $item_name = $this->CRUDModel->get_where_row('invt_items', array('itm_id' => $item_id[$row]));
            $result_array = array(
                'item_name'     => $item_name->itm_name,
                'work_id'       => $worderId,
                'days'          => $days[$row],
                'quantity'      => $quantity[$row],
                'price'         => $unit_price[$row],
            ...
            $this->CRUDModel->insert('invt_work_order_details', $result_array);
        endforeach;
    }
