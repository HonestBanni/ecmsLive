...
<h2 align="center" style="border-bottom:1px solid #ccc;">Update Student (Inter Level)</h2>   
    <div class="row cols-wrapper">
        ...
        <div class="form-group col-md-3">
            ...
            echo '<option type="text" value=""></option>';
                ...
                <option>&larr; Select &rarr;</option>
                 <?php
                    $b = $this->db->query("SELECT * FROM prospectus_batch WHERE  `batch_name` LIKE  '%FA%'");
        ...
        <option type="text" value="<?php echo $rec->rseat_id;?>"><?php echo $rec->name;?></option>
            ...
            <div class="form-group col-md-2">                       
                <input type="submit" class="btn btn-theme" name="submit" style="margin-top:23px;" value="Update Student">
            </div> 
    ...
    <div class="form-group col-md-3">
        ...
        <?php if($s_status_id != 1 && !empty($student_name)):?>
        <input type="text" name="student_name" value="<?php echo $student_name;?>" class="form-control" readonly>
        <?php else: ?>
        <input type="text" name="student_name" value="<?php echo $student_name;?>" class="form-control" required>
        <?php endif; ?>
    </div>                                          
    <div class="form-group col-md-3">
...
<div class="form-group col-md-3">  
    ...
    <?php  
    $country = $this->get_model->get_by_id('country',array('country_id'=>$country_id));
    if($country):
    foreach($country as $c):
    ?>
        <input type="text" name="country_id" value="<?php echo $c->name;?>" class="form-control" id="country">
            <input type="hidden" name="country_id" value="<?php echo $c->country_id;?>" id="country_id">
    <?php 
        endforeach;
        else:
    ?>
...
<div class="form-group col-md-3">  
 ...
 <?php  
    $domicile = $this->get_model->get_by_id('domicile',array('domicile_id'=>$domicile_id));
    if($domicile):
    foreach($domicile as $dom):
    ?>
        <input type="text" name="domicile_id" value="<?php echo $dom->name;?>" class="form-control" id="domicile">
            <input type="hidden" name="domicile_id" value="<?php echo $dom->domicile_id;?>" id="domicile_id">
    <?php 
        endforeach;
        else:
    ?>
...
<?php 
    $district = $this->get_model->get_by_id('district',array('district_id'=>$district_id));
    if($district):
    foreach($district as $dis):
    ?>
        <input type="text" name="district_id" value="<?php echo $dis->name;?>" class="form-control" id="district">
        <input type="hidden" name="district_id" value="<?php echo $dis->district_id;?>" id="district_id">
    <?php 
        endforeach;
        else:
    ?>
...
<select class="form-control" type="text" name="hostel_required">
       <option value="<?php echo $row->hostel_required;?>"><?php echo $row->hostel_required;?></option>
...
<div class="form-group col-md-12">                       
    <input type="submit" class="btn btn-theme" name="submit" value="Update Student">
                ...
                <br><br>
                <h3 align="center">Student Academic Record Details<hr><span style="folat:right"><a href="admin/new_student_academic_record/<?php echo $id;?>" class="btn btn-large btn-primary">Add Academic Record</a></span></h3>
...
<td><a class="btn btn-theme btn-sm" href="admin/update_new_academic_record/<?php echo $rec->serial_no;?>">Update</a></td>  
                            <!--
 ...
 <div class="alert alert-info col-md-12">
           <strong>Subject Wise Grade List For O Level Students Only.</strong>
        </div>
    <?php
      $where1 = array('student_grades.student_id'=>$this->uri->segment(3));    
      $grade = $this->get_model->getstudents_grade($where1);
        if(@$grade):
                ?>
                <article class="contact-form col-md-12 col-sm-7">
                    ...
                    <tbody>
                        ...
                        <?php
                        $total_percentage += $eRow->grade_value;
                        endforeach;  
                        $res = ($total_percentage/$count)*100; 
                        ?>
    <tr>
        <td></td>                    
        <td><strong>Percentage: </strong></td>                    
        <td colspan="2"><strong><?php echo $res = round($res,3);?>%</strong></td>
                        </tr>                    
                        </tbody>
                </table>
          </article> 
